 ...
 */
const findPlaylistById = function(playlist_id) {
 ...
 */
const findPlaylistByName = function(name) {
...
let getSongsByPlaylistIdentifier = function(identifier: string, identifier_type = null) {
	let valid_identifiers = ["playlist_id", "name"];
	...
	if (identifier_type === null) {
		...
		if (!valid_identifiers.includes(identifier_type)) {
			...
			//Valid identifier type sent, just run the query
			return getSongsByPlaylist(identifier, identifier_type);
		}
	}
};
...
let searchForSongs = function(name, max_songs = 10) {
	...
	let clauses = [];
	let prepped_args = [];
	let args = name.split(" ").filter((word) => {
		return word.trim() !== "";
	});
...
const findImageById = function(image_id) {
	if (!isInt(image_id)) {
...
const findImageByHashId = function(hash_id) {
...
let insertCommands = function(commands) {
	const insert = DB.prepare(
		`INSERT INTO ${COMMAND_TABLE} (command, default_access) VALUES (@command, @default_access) ON CONFLICT(command) DO UPDATE SET default_access=@default_access`
	);
